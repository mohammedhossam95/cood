...
class AddUserSocialAccountCubit extends Cubit<AddUserSocialAccountState> {
  ...
  Future<void> addUserSocialAccount(AddAccountParams param) async {
    ...
    try {
      changeLoadingView();
      final result = await addUserSocialAccountuseCase.call(param: param);
      emit(
        result.fold(
          (failure)=>AddUserSocialAccountFailure(errorMessage: failure.toString()),
          (resp)=>AddUserSocialAccountSuccerss(response: resp)
          )
      );
    } catch (e) {
